...
var overviewPage = {
    init: function() {
        ...
        // Announcement feed
        $.ajax({
            url:"https://ajax.googleapis.com/ajax/services/feed/load?v=2.0&q=http://www.infocoin.info/?feed=rss2",
            dataType: 'jsonp'
    ...
    $("#recipients").append((
        ...
        +  '<div id="recipient[count]" class="recipient"> \
            <div class="flex-right"> \
                <label for="pay_to[count]" class="recipient">支付给:</label> \
                <input id="pay_to[count]" class="pay_to input_box" data-title="The address to send the payment to  (e.g. SXywGBZBowrppUwwNUo1GCRDTibzJi7g2M)" placeholder="Enter a IFC address (e.g. SXywGBZBowrppUwwNUo1GCRDTibzJi7g2M)" maxlength="128" oninput="base58.check(this);" onchange="$(\'#label[count]\').val(bridge.getAddressLabel(this.value));"/> \
            ...
            <div class="flex-left"> \
                ...
                <select id="unit[count]" class="unit button is-inverse has-fixed-icon"  style="margin-left:10px; height:43px; width:100px;" onchange="unit.format(\'#amount[count]\', $(this).val());"> \
                    <option value="0" title="IFC"                    ' + (unit.type == 0 ? "selected" : "") + '>IFC</option> \
                    <option value="1" title="Milli-IFC (1 / 1000)"   ' + (unit.type == 1 ? "selected" : "") + '>mIFC</option> \
                    <option value="2" title="Micro-IFC (1 / 1000000)"' + (unit.type == 2 ? "selected" : "") + '>&micro;IFC</option> \
                    <option value="3" title="IFCshi (1 / 100000000)" ' + (unit.type == 3 ? "selected" : "") + '>IFCshi</option> \
                </select> \
            </div> \
