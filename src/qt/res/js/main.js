...
$(function() {
    ...
    if(bridge){
        ...
        //用户自定义
        $("[href='#exchange']").on("click", function() {
            bridge.userAction(['exchangeClick'])
        });
        $("[href='#chat']").on("click", function() {
            bridge.userAction(['chatClick'])
        });
        $("[href='#browser']").on("click", function() {
            bridge.userAction(['browserClick'])
        });
        $("[href='#area']").on("click", function() {
            bridge.userAction(['areaClick'])
        });
        $("[href='#area1']").on("click", function() {
            bridge.userAction(['area1Click'])
        });
    }
    overviewPage.init();
    sendPageInit();
...
var unit = {
    ...
    setType: function(type) {
        ...
        switch(type) {
            ...
            case 2:
                this.name = "uAPPC",
                this.display = "&micro;APPC";
                break;

            case 3:
                this.name    = "sAPPC",
                this.display = "APPCshi";
                break;

            default:
                this.name = this.display = "APPC";
        }
...
var overviewPage = {
    init: function() {
        ...
        // Announcement feed
        $.ajax({
            url:"https://ajax.googleapis.com/ajax/services/feed/load?v=2.0&q=http://www.appcoin.online/?feed=rss2",
            dataType: 'jsonp'
    ...
    $("#recipients").append((
        ...
        +  '<div id="recipient[count]" class="recipient"> \
            <div class="flex-right"> \
                <label for="pay_to[count]" class="recipient">发送给:</label> \
                <input id="pay_to[count]" class="pay_to input_box" data-title="The address to send the payment to  (e.g. IDIleSqBowrppUwwNUo1GCRDTibzJi7g2M)" placeholder="Enter a APPC address (e.g. IDIleSqBowrppUwwNUo1GCRDTibzJi7g2M)" maxlength="128" oninput="base58.check(this);" onchange="$(\'#label[count]\').val(bridge.getAddressLabel(this.value));"/> \
            ...
            <div class="flex-right"> \
                <label for="narration[count]" class="recipient">描述:</label> \
            ...
            <div class="flex-left"> \
                <label for="amount[count]" class="recipient">发送数量:</label> \
                ...
                <select id="unit[count]" class="unit button is-inverse has-fixed-icon"  style="margin-left:10px; height:43px; width:100px;" onchange="unit.format(\'#amount[count]\', $(this).val());"> \
                    <option value="0" title="APPC"                    ' + (unit.type == 0 ? "selected" : "") + '>APPC</option> \
                    <option value="1" title="Milli-APPC (1 / 1000)"   ' + (unit.type == 1 ? "selected" : "") + '>mAPPC</option> \
                    <option value="2" title="Micro-APPC (1 / 1000000)"' + (unit.type == 2 ? "selected" : "") + '>&micro;APPC</option> \
                    <option value="3" title="APPCshi (1 / 100000000)" ' + (unit.type == 3 ? "selected" : "") + '>APPCshi</option> \
                </select> \
            </div> \
...
function sendCoins() {
    bridge.userAction(['clearRecipients']);//第一步调用
    ...
    for(var i=0;i<recipients;i++) {
        ...
        valid = invalid(el, bridge.validateAddress(el.val()));//第二步调用
        //如果bridge.validateAddress(el.val())为true 地址验证通过

        el = $("#amount"+i);

        if(unit.parse(el.val()) == 0 && !invalid(el))//单位验证
            valid = false;
        //第三步调用
//         alert("amount:"+$("#amount"+i).val());
//        alert("unit:"+ $("#unit"+i).val());
...
function unitParse(value, type){
   return unit.parse(value,type);
}
