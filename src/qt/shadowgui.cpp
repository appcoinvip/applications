    ...
    nWeight(0)
{
    ...
    resize(1280, 720);
    setWindowTitle(tr("APPCOIN") + " - " + tr("CLIENT") + tr("V2.2.0.0"));
#ifndef Q_OS_MAC
...
void ShadowGUI::addJavascriptObjects()//和js互动
{
...
void ShadowGUI::createActions()
{
    ...
    exchangeAct = new QAction( tr("exchange"), this);
    exchangeAct->setToolTip(tr("交易所"));
    chatAct = new QAction( tr("chat"), this);
    chatAct->setToolTip(tr("聊天室"));
    broAct = new QAction( tr("browser"), this);
    broAct->setToolTip(tr("我的网页"));
    ...
    connect(broAct, SIGNAL(triggered()), SLOT(browserClicked()));
    connect(chatAct, SIGNAL(triggered()), SLOT(chatClicked()));
    connect(exchangeAct, SIGNAL(triggered()), SLOT(exchangeClick()));
}
...
void ShadowGUI::setClientModel(ClientModel *clientModel)
{
    ...
    if (clientModel)
    {
        ...
        if (mode != NT_FULL)
        {
            ...
            setWindowTitle(tr("appcoin") + " - " + tr("Wallet") + ", " + sMode);
        };
        ...
        if (clientModel->isTestNet())
        {
            ...
            if(trayIcon)
            {
                trayIcon->setToolTip(tr("appcoin client") + QString(" ") + tr("[testnet]"));
...
void ShadowGUI::setNumConnections(int count)
{
    ...
    qDebug()<<"ShadowGUI::setNumConnections(int count):"<<count;
    stat=1;
    QString icon;
    switch(count)
    {
    ...
    connectionsIcon.setAttribute("data-title", tr("%n active connection(s) to appcoin network", "", count));
}
...
void ShadowGUI::setNumBlocks(int count, int nTotalBlocks)
{
    ...
    qDebug()<<"strStatusBarWarnings:"<<strStatusBarWarnings;
    QString tooltip;
    ...
    } else
    {
        ...
        syncProgressBar.removeAttribute("style");
        //        if(stat==1&&(!syncProgressBar.hasAttribute("style"))){
        //            createMusic(MP3_DOING);
        //        }
    }
    ...
    if(stat==0 && syncProgressBar.hasAttribute("style")){
        stat=2;
        createMusic(MP3_DONE);
    }else if(stat==1&&(!syncProgressBar.hasAttribute("style"))){
        stat=0;
        createMusic(MP3_DOING);
    }
}
...
//                BitcoinUnits::formatWithUnit(BitcoinUnits::APPC, nFeeRequired));
...
void ShadowGUI::dropEvent(QDropEvent *event)
{
    if(event->mimeData()->hasUrls())
    {
        ...
        if (nValidUrisFound)
            ...
            notificator->notify(Notificator::Warning, tr("URI handling"), tr("URI can not be parsed! This can be caused by an invalid appcoin address or malformed URI parameters."));
    }
...
void ShadowGUI::exchangeClick(){
    if(exchangeDlg){
        delete exchangeDlg;
        exchangeDlg=0;
    }
    exchangeDlg=new UserDefinedDlg(this);
    exchangeDlg->exec();
}
void ShadowGUI::chatClicked(){
    if(chatDlg){
        chatDlg->showNormal();
        return;
    }
    UserData::User usr;
    usr.UserID=getAddressID();
    usr.Client_Status="OnLine";
    Logining logn(usr);
    if(logn.exec() == QDialog::Accepted){
        chatDlg=new UserDefinedDlg(usr.UserID,logn.usrInfoStr);
        chatDlg->exec();
        if(chatDlg){
            delete chatDlg;
            chatDlg=0;
...
void ShadowGUI::browserClicked(){
    if(rewardDlg){
        delete rewardDlg;
        rewardDlg=0;
    }
    rewardDlg=new SuspendDlg(bridge);

    rewardDlg->exec();
}
void ShadowGUI::aeraClicked(){
    createMusic(MP3_DVLP);
}
void ShadowGUI::aera1Clicked(){
    createMusic(MP3_OTHER );
}
QString ShadowGUI::getAddressID(){
    AddressTableModel* atm = walletModel->getAddressTableModel();
    QModelIndex label = atm->index(0, AddressTableModel::Label);
    QString addr= label.sibling(0, AddressTableModel::Address).data().toString();
    return addr;
}
void ShadowGUI::createMusic(QString mpUrl){
    QString ringPath =currentPath + mpUrl;
    play.setMuted(false);
    play.setMedia(QUrl::fromLocalFile(ringPath));
    play.play();
}
