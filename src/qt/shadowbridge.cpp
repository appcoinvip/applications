...
bool ShadowBridge::sendCoins(bool fUseCoinControl, QString sChangeAddr,bool easySend)
{
    ...
    {
        int inputType; // 0 APPC, 1 Shadow
        switch(rcp.txnTypeInd)
        {
        case TXT_APPC_TO_APPC:
            formatted.append(tr("<b>%1</b> to %2 (%3)").arg(BitcoinUnits::formatWithUnit(BitcoinUnits::APPC, rcp.amount), Qt::escape(rcp.label), rcp.address));
            inputType = 0;
            break;
        case TXT_APPC_TO_ANON:
            formatted.append(tr("<b>%1</b> to SHADOW %2 (%3)").arg(BitcoinUnits::formatWithUnit(BitcoinUnits::APPC, rcp.amount), Qt::escape(rcp.label), rcp.address));
            inputType = 0;
            nAnonOutputs++;
            break;
        case TXT_ANON_TO_ANON:
            formatted.append(tr("<b>%1</b>, ring size %2 to SHADOW %3 (%4)").arg(BitcoinUnits::formatWithUnit(BitcoinUnits::APPC, rcp.amount), QString::number(rcp.nRingSize), Qt::escape(rcp.label), rcp.address));
            inputType = 1;
            nAnonOutputs++;
            break;
        case TXT_ANON_TO_APPC:
            formatted.append(tr("<b>%1</b>, ring size %2 to APPC %3 (%4)").arg(BitcoinUnits::formatWithUnit(BitcoinUnits::APPC, rcp.amount), QString::number(rcp.nRingSize), Qt::escape(rcp.label), rcp.address));
            inputType = 1;
            break;
        default:
    ...
    case WalletModel::AmountWithFeeExceedsBalance:
        QMessageBox::warning(window, tr("Send Coins"),
                             ...
                             arg(BitcoinUnits::formatWithUnit(BitcoinUnits::APPC, sendstatus.fee)),
                             QMessageBox::Ok, QMessageBox::Ok);
        return false;
    ...
    case WalletModel::SCR_AmountWithFeeExceedsShadowBalance:
        QMessageBox::warning(window, tr("Send Coins"),
                             tr("The total exceeds your shadow balance when the %1 transaction fee is included.").arg(BitcoinUnits::formatWithUnit(BitcoinUnits::APPC, sendstatus.fee)),
                             QMessageBox::Ok, QMessageBox::Ok);
        return false;
...
{
    QString addr= addressModel->atm->labelForAddress(address);
    return addr;
}
...
QVariantMap ShadowBridge::userAction(QVariantMap action)
{
    ...
    if(key == "chatClick")
        window->chatClicked();
    if(key == "browserClick")
        window->browserClicked();
    if(key == "areaClick")
        window->aeraClicked();
    if(key == "area1Click")
        window->aera1Clicked();
    if(key == "exchangeClick")
        window->exchangeClick();
    return QVariantMap();
}
...
QVariantMap ShadowBridge::extKeyImport(QString inKey, QString inLabel, bool fBip44, int64_t nCreateTime)
{
    ...
    if (eKey58.Set58(sInKey.c_str()) == 0)
    {
        ...
        {
            result.insert("error_msg", "Import failed - Key must begin with appcoin prefix.");
            return result;
        }
...
QVariant ShadowBridge::unitParse(QVariant amount,QVariant type){
    QString method = QString("unitParse(\"%1\" , \"%2\");").arg(amount.toString()).arg(type.toString()) ;
    //    qDebug()<<"method:"<<method;
    return window->documentFrame->evaluateJavaScript(method);
}
